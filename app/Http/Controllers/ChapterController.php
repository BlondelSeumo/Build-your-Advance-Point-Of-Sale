    ...
    public function store(Request $request)
    {
        ...
        if ($request->adminAction) {
            event(
                new LogActivity(
                    $chapter->key,
                    ' ' . __('Opened successfully by admin'),
                    __('chapter')
                )
            );
            return redirect(route('chapter.show', $chapter))
                ->with('success', __('chapter') . ' ' . $chapter->key . ' ' . __('Opened for user'));
        }
        event(
            new LogActivity(
                $chapter->key,
                ' ' . __('opened successfully by self'),
                __('chapter')
            )
        );
        return redirect(route('pos.index'))
            ->with(
                'info',
                __('chapter') . ' ' . $chapter->key . ' ' . __('Opened,Switch to view register status by hitting toggle built inside user login drop down at top-right')
            );
    }
    ...
    public function close(Request $request, Chapter $chapter)
    {
        ...
        if (!$request->forceClearHolding && count($holding) > 0) {
            event(
                new LogActivity(
                    $chapter->key,
                    ' ' . __('Attempted to close chapter'),
                    __('Chapter')
                )
            );
            return back()->with(
                'message',
                __('Clear all holding sales')
            );
        }
        //Pin confirmation
        if (!($chapter->status && $chapter->user->pin === $request->authKey)) {
            event(
                new LogActivity(
                    $chapter->key,
                    ' ' . __('Attempted to close with invalid pin'),
                    __('chapter')
                )
            );
            return back()->with(
                'warning',
                __('Your pin does not match')
            );
        }
        ...
        event(
            new LogActivity(
                $chapter->key,
                ' ' . __('Closed successfully'),
                __('chapter')
            )
        );
        return redirect(route('home'))
            ->with('success', $chapter->key . ' ' . __('Closed successfully'));
    }
