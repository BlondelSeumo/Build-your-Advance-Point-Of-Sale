...
class RefundController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            event(
                new LogActivity(
                    $refund->reference,
                    ' ' . __('Order refunded successfully'),
                    __('refund')
                )
            );
            DB::commit();
            ...
            $response = [
                'reference' => $refund->id,
                'message' => __('Refund successfully'),
            ];
        } catch (Exception $e) {
            DB::rollBack();
            $response = [
    ...
    public function destroy(Refund $refund)
    {
        ...
        event(
            new LogActivity(
                $refund->reference,
                ' ' . __('Refund order removed'),
                __('refund')
            )
        );
        return redirect(route('refund.index'))
            ->with('success', __('Refund order deleted'));
    }
    ...
    public function confirmSale(Request $request)
    {
        ...
        if ($sale) {
            event(
                new LogActivity(
                    $sale->reference,
                    ' ' . __('Sale order confirmed for refund'),
                    __('refund')
                )
            );
            return response()->json(
                [
                    'sale_id' => $sale->id,
                    'type' => 'success',
                    'message' => __('Sale order confirmed'),
                ],
                200
            );
        }
        event(
            new LogActivity(
                'sale order',
                ' ' . __('Order not confirmed for refund'),
                __('refund')
            )
        );
        return response()->json(
