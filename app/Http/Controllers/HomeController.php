    ...
    public function quickMail(Request $request)
    {
        ...
        if (Mail::failures()) {
            event(
                new LogActivity(
                    $request->email,
                    __('Failed to send mail') . ' ' . $request->subject,
                    __('Quick mail')
                )
            );
            return back()->with('warning', __('Something went wrong try again !'));
        }
        event(
            new LogActivity(
                $request->email,
                __('Mail sent successfully') . ' ' . $request->subject,
                __('Quick mail')
            )
        );
        return back()->with('success', __('Great successfully sent'));
    }
