...
class TaxController extends Controller
{
    ...
    public function store(TaxRequest $request)
    {
        ...
        event(
            new LogActivity(
                $tax->name,
                ' ' . __('New tax method created'),
                __('tax')
            )
        );
        return response()->json(
            ['message' => __('Tax method created')],
            200
        );
    }
    ...
    public function update(TaxRequest $request, Tax $tax)
    {
        ...
        event(
            new LogActivity(
                $tax->name,
                ' ' . __('Tax method updated'),
                __('tax')
            )
        );
        return response()->json(
            ['message' => __('Tax method updated')],
            200
        );
    }
    ...
    public function destroy(Tax $tax)
    {
        ...
        if ($tax->purchases->count() > 0) {
            return back()->with('warning', __('Tax method is being used'));
        }
