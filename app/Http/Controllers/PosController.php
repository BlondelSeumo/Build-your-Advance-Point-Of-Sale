...
use App\Cart;
use App\Http\Requests\PosHoldOnRequest;
use App\Http\Requests\PosScanRequest;
use App\Http\Requests\PosSearchRequest;
use App\Product;
...
class PosController extends Controller
{
    ...
    public function itemEdit(request $request)
    {
        if (auth()->user()->pin !== $request->item['userPin']) {
            return response()->json(
                ['type' => 'warning', 'message' => __('Invalid auth PIN')],
                200
            );
        }
    ...
    public function showCart(Request $request)
    {
        ...
        if ($request->holdOnOrderKey) {
            ...
            unset(
                $holdOnOrders[array_search($request->holdOnOrderKey, $holdOnOrders)]
            );
    ...
    public function qtyChange(Request $request)
    {
        ...
        if ($stack <= $request->qty) {
            return response()->json(
                [
                'message' => $stack . ' ' . __('Available only in stack'),
                'type' => 'warning',
                ],
                200
            );
     ...
     *
     * @param PosScanRequest $request the request
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function scan(PosScanRequest $request)
    {
     ...
     *
     * @param PosSearchRequest $request the request
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function search(PosSearchRequest $request)
    {
     ...
     * Hold Orders
     *
     * @param PosHoldOnRequest $request The request
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function holdOn(PosHoldOnRequest $request)
    {
        ...
        return response()->json(
            ['type' => 'success', 'message' => __('Order holed successfully')],
            200
        );
    }
}
