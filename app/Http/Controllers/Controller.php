...
class Controller extends BaseController
{
    ...
    {
        if ('default_img/no_image.png' !== $imageUrl && file_exists(storage_path($imageUrl))
        ) {
            unlink(storage_path($imageUrl));
        }
    }
    ...
    {
        return $request->code ?? random_int(11, 99) . time();
    }
    ...
    /**
     * Gives permission keys for groups
     *
     * @param mixed $request The request
     *
     * @return array
    ...
    {
        $languageFile = base_path('resources/lang/' . $locale . '.json');
