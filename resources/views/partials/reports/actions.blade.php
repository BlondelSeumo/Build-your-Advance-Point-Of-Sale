<form action="{{ route('report.store') }}" method="post">
    ...
    <div class="form-group pb-0 pt-0">
        ...
        <button type="submit" class="btn btn-sm btn-success">
            <i class="fa fa-save btn-submint fa-lg" aria-hidden="true"></i> {{ __('Save Report') }}
        </button>
    </div>
</form>
<form class="form" action="{{ $genLink }}" method="post">
    ...
    <div class="form-group col-md-12">
        ...
        <a href="{{ $backLink }}" class="btn btn-info btn-sm">{{ __('Back') }}</a>
        <button class="btn btn-default btn-sm" type="submit">{{ __('Print') }}</button>
    </div>
</form>
